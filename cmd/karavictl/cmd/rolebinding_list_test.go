...
func TestRolebindingList(t *testing.T) {
	...
	t.Run("it lists a rolebinding", func(t *testing.T) {
		defer afterFn()
		var gotCalled bool
		CreateTenantServiceClient = func(_ string, _ bool) (pb.TenantServiceClient, io.Closer, error) {
			return &fakeTenantServiceClient{
				ListBindRoleFn: func(_ context.Context, _ *pb.ListBindRoleRequest, _ ...grpc.CallOption) (*pb.ListBindRoleResponse, error) {
					gotCalled = true
		...
		rootCmd := NewRootCmd()
		listRoleBindingCmd := NewListRoleBindingCmd()

		listRoleBindingCmd.SetOutput(&gotOutput)
		rootCmd.SetArgs([]string{"rolebinding", "list"})
		rootCmd.Execute()

		if !gotCalled {
			t.Error("expected ListBindRole to be called, but it wasn't")
		}
	})
	...
	t.Run("it handles server errors", func(t *testing.T) {
		...
		cmd.SetArgs([]string{"rolebinding", "list"})
